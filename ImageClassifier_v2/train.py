...
save_dir = '.'
arch = 'vgg19'
model = all_models.AllModel(arch)
learning_rate = 0.001
hidden_units = None
num_epoch  = 5
...
parser.add_argument('--num_epoch', help='Number of training iterations')
## Training with GPU
parser.add_argument("-g","--gpu",help ="Training with GPU or CPU",type=str,action='store',default='gpu')
...
for e in range(num_epoch):
    ...
    for images, labels in trainloader:
        ...
        if step % 41 == 0 or step == 1:
            print("Epoch: {}/{} ... ".format(e+1,num_epoch),
                  "Training Step: {}/{} ... ".format(step,len(trainloader)),
                  "Time elapsed: {:.3f} seconds".format((time.time()-begin)/3))
        step += 1

    else: # End training iteration, begin validation
        print("Epoch: {}/{} ... ".format(e+1,num_epoch), "Training complete.")
        print("Epoch: {}/{} ... ".format(e+1,num_epoch), "Validating... ")
        test_loss = 0
        accuracy = 0
        with torch.no_grad():
            model.eval()
